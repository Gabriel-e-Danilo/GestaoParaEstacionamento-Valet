...
public interface IRepositorioVaga : IRepositorio<VagaEstacionamento>
{
    ...
    Task<VagaEstacionamento?> SelecionarPorTicketAtivoAsync(int ticket);
    Task<VagaEstacionamento?> SelecionarPorPlacaAtivaAsync(string placa);


    Task<List<(string Placa, int Ticket, string IdentificadorVaga, 
               string Zona, DateTime DataHoraEntrada)>> 
